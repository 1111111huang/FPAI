...
| ID | Status | Description | Comments |
|---|---|---|---|
| US#107 | active | **League-specific model with registry-driven general-purpose fallback**: `ForecastService.forecast_upcoming()`'s league branch must consult `config/competitions.yaml` (the Phase 14a registry, already used by `model_manager.py`/`main.py` at training time but not here) before loading model context. If the requested `league`/`competition_id` resolves to a `competition_specific` entry with a trained model set, use it; otherwise route to `general_purpose` automatically instead of requiring the caller (agent or app) to already know not to ask. **Acceptance:** a call for a non-registered league (e.g. `league="La Liga"`) returns `prediction_basis: "market_odds_only"` with an honest `general_purpose` caveat, not a silently cold-started `team_history_and_market` result; a call for `league="E0"` is unaffected (still resolves `competition_specific`); regression test covers both branches plus the existing E0 behavior. | Size M · Depends on: Phase 14a (US#87–90, registry already exists) |
