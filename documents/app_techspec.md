...
**Test counts as of the most recent completed stories (W45–W53, 2026-07-21):** backend (`tests/` + `app/backend/tests/` + `scripts/`) 548 passed / 23 skipped, zero regressions since W51's 543/23/1 baseline (W52 added 4 backend tests: TTL dedup, HTTPError→503 degrade, in-flight concurrent-request dedup, TTL-expiry re-fetch; W53 added no backend tests, frontend-only). Frontend (Vitest + RTL) grew from 42 to 48 tests across 9 files (`tsc --noEmit` clean) — W52 added 2 (`BetTracker.fixtureError.test.tsx`), W53 added 4 (`MatchUI.precompute.test.tsx`).
