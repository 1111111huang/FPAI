...
| 11 — More Bugs Found By Actually Using The App | W45–W51 | ✅ Implemented (Section 9.10) |

**Test counts as of the most recent completed stories (W45–W51, 2026-07-21):** backend (`tests/` + `app/backend/tests/` + `scripts/`) 543 passed / 23 skipped / 1 deselected, zero regressions since W44's 533/1/1 baseline (the skip count reverted to 23 in this worktree; not investigated as part of this story, same caveat noted at W44). Frontend (Vitest + RTL) unchanged at 42 tests across 7 files (`tsc --noEmit` clean) — W51 only touched `MatchUI.tsx` for a comment, no test-affecting change.
