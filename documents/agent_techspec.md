...
**Verification.** `tests/test_agent_live_wait_conditional.py` (18 tests, rewritten: the `no_bet`-promotion tests from the original design carry over largely unchanged in spirit, plus a `direct_bet`-candidates-are-never-touched regression test replacing the old override test, plus 5 new tests for the pick-switching function — the exact draw/btts scenario, no-switch when the conditional isn't more probable, no-switch when no eligible alternative exists, no-switch for an ineligible market, no-switch when the pick isn't a `direct_bet`), `tests/test_agent_config.py` (+2: default config's real values, posture configs stay `None`). Threaded through `graph.py`'s `_build_recommendation()` call site. Full suite: 1084 passed, 1 skipped, zero regressions.
