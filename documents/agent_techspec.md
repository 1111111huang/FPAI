...
**Mechanics** (`_promote_favorite_to_conditional_for_live_wait()`, `src/agent/schema.py`). Unlike every downgrade pass in this file, this one *promotes*: it can turn `no_bet` into `conditional` (a gap the value-edge check alone wouldn't fill, since the premise is the current price/edge doesn't need to clear the bar yet — only the price this is expected to drift to needs to) and can override an existing `direct_bet` too, per the user's explicit stated preference (given both a `direct_bet` and this condition for the same pick, prefer waiting for the better number on a likely winner rather than betting the worse one now). A candidate qualifies when all of:

1. `(market, selection)` is in the same `_CONDITIONAL_ELIGIBLE_MARKETS` set A54 already restricts `conditional` to (`btts/yes`, `total_goals/over_2.5`, the three corners-over markets) — waiting has to be a real directional strategy, not a coin flip.
2. `current_odds >= live_wait_min_odds` — new `AgentConfig.live_wait_min_odds: float | None = None` (opt-in; only `config/agent_config.yaml`, the live production default, sets a real value — `1.6667` decimal / −150 American, the user's own stated floor: "anything over −150 is good enough"). Deliberately *looser* than the general `conditional` floor A66 already enforces (prod default `1.71`/−140) — this rule runs after A66's own pass so a newly-promoted −150 pick isn't immediately undone by a floor built for a different (pre-match-drift-only) premise.
3. `current_odds < live_wait_target_odds` — new `AgentConfig.live_wait_target_odds: float = 2.0` (decimal / +100 American, the price this strategy assumes it's waiting for). **Correctness fix found live while formalizing this**: the first version of this rule had no such check, so a `direct_bet` already priced *better* than the target (e.g. already +150) could get wrongly promoted to `conditional` — recommending waiting for a worse price than already available. This check makes that structurally impossible.
4. `ml_probability - implied(live_wait_target_odds) >= min_value_edge` — direct user refinement, same day: *"when there is no direct bet, a conditional bet can be recommended if the target odd (+100) clears the edge requirement."* Replaces an original bare `ml_probability > 0.5` check. For an existing `direct_bet`, this is automatically satisfied whenever checks 2–3 hold (its own edge at a shorter current price already cleared `min_value_edge`, and edge only grows as price lengthens toward the target) — so the uniform check changes nothing for a legitimate `direct_bet` override while genuinely gating the `no_bet` case the refinement was about.

Runs in `extract_recommendation()`'s pipeline after A66's floor check and the ceiling check (ordering reason in check 2 above), before A52's `target_odds` computation — so a newly-promoted candidate gets a real target price computed for it too, same as any other `conditional` pick. `live_wait_min_odds=None` (the default for every posture config except the live production one) disables the whole rule; every other caller/test is unaffected by construction.

**Verification.** `tests/test_agent_live_wait_conditional.py` (14 tests: disabled-by-default, promotes `no_bet`, overrides `direct_bet`, rejects a non-favorite probability, rejects too-short a price, rejects ineligible markets, leaves an already-`conditional` candidate alone, gets a real `target_odds`, the key floor-conflict case — survives the stricter general conditional floor at a price between the two thresholds — plus the edge-at-target gate accepted/rejected at the threshold and the two `direct_bet`-already-better-than-target correctness-fix cases), `tests/test_agent_config.py` (+2: default config's real values, posture configs stay `None`). Threaded through `graph.py`'s `_build_recommendation()` call site. Full suite: 1080 passed, 1 skipped, zero regressions.
