...
### 27.3 `overall`/`markets` reconciliation — `_reconcile_overall_with_markets()` (`src/agent/schema.py`, A65)

Direct user report, live on the deployed Dashboard: a card's top-right badge read "Direct Bet" (`overall`) over a market rendering as "WAIT ≥ 1.13" (that market's own `recommendation_type: "conditional"`) — the badge and the market it described told two different stories. Root cause: `overall` is the LLM's own top-level self-report, written before `extract_recommendation()`'s three code-enforced downgrade passes (Section 19.2/A28's `_downgrade_direct_bet_with_null_odds`, A29's `_downgrade_direct_bet_outside_odds_bounds`, A54's `_restrict_conditional_to_eligible_markets`) ever run — none of them touched `overall`, so a market downgraded by any of them could leave `overall` claiming a stronger state (`direct_bet` > `conditional` > `no_bet`) than any surviving market actually supports. Confirmed directly against this repo's own existing test fixtures: `tests/test_agent_odds_bounds.py`'s downgrade tests asserted the market-level change but had never checked `overall` alongside it — it stayed `"direct_bet"` in every one of them, unnoticed until this live report.

`_reconcile_overall_with_markets(data)` runs last in the downgrade sequence (after `_compute_target_odds`) — ranks `overall`/`recommendation_type` on the same `direct_bet > conditional > no_bet` scale (`insufficient_data` ranked below all three, but never reachable from a market-level value) and caps `data["overall"]` at the strongest rank any market in `data["markets"]` still has, appending a `limitations` note when it actually downgrades (matching the three sibling passes' own convention). Downgrade-only, same direction as every pass above — never raises `overall` above what the LLM itself reported, only lowers it to match reality. A no-op when `markets` is empty (nothing to reconcile against — that shape belongs to `graph.py`'s own no-forecast short-circuit, which returns before `extract_recommendation()` is ever called).

**Known limitation, not addressed here:** a recommendation cached under the pre-A65 code keeps its stale `overall` — `agent_config_hash` (`RecommendationCache`'s key, Section 6.3 of `app_techspec.md`) hashes `AgentConfig`'s tunable fields only, not this code's own version, so W151's `already_fresh()` dedup (Section 27.2) will treat an unchanged-odds fixture as still fresh and skip regenerating it even after this fix ships. A stale card self-corrects once its odds move (the next EOD/T-30 pass no longer finds `already_fresh()` true) or via a manual `/api/admin/pregenerate-recommendations` trigger — not fixed with a cache-schema-version bump here, since a one-off code fix bumping every cache key would be speculative machinery for a problem that will otherwise resolve itself on the next real odds movement.

**Follow-up, same investigation (W153, `documents/app_user_stories.md`):** A65 alone caps `overall` at the strongest recommendation_type across *all* markets — correct for `isActionable()`/`dashboardMetrics.ts`'s match-wide "is anything actionable" aggregate, but not a full guarantee that the frontend's badge matches the *one* market it actually displays (`bestMarket()` picks by highest edge among actionable markets, which can be a `conditional` market even when a lower-edge `direct_bet` market also exists on the same match). W153 closes that gap on the frontend side — `MatchUI.tsx`'s badge/verdict render sites now derive from `bestMarket(match)?.recommendationType` directly, falling back to `match.overall` only when there's no market to show at all. This is the correct source of truth for a per-card badge regardless of what A65 does server-side; A65 remains necessary for the separate match-wide aggregate.
...
New `AgentConfig.min_conditional_odds_threshold` (default `1.5` decimal, roughly -200 American) — added the same way `min_odds_threshold`/`max_odds_threshold` already are: a dataclass field, an `agent_config.yaml`/`agent_config_deepseek.yaml` entry, threaded through `extract_recommendation()`'s signature, and (new) included in `compute_agent_config_hash()` (Section 6.3 of `app_techspec.md`). `_downgrade_conditional_below_floor(data, min_conditional_odds_threshold)` runs after A29's/A54's downgrade passes and before A52's `target_odds` computation — any market still `conditional` with `current_odds < floor` (null is out of scope, same precedent as A29's own bounds check) downgrades to `no_bet`. One rule covers two distinct problems: a genuinely too-short real price (waiting for it to improve isn't realistic) and a degenerate non-price like `0.0` (which fails any positive floor trivially) — no separate null-style validity check needed.

**Structural consequence:** `min_odds_threshold` (1.2) is always below `min_conditional_odds_threshold` (1.5) — so A29's own *floor*-downgrade case (a `direct_bet` whose `current_odds` is too low) can now never survive as `conditional` end-to-end; it always cascades one step further to `no_bet`. Four pre-existing tests whose fixtures relied on observing that now-impossible intermediate state were updated to exercise what they were actually testing via A29's *ceiling* case or an LLM-organic `conditional` call instead (`tests/test_agent_odds_bounds.py`, `test_agent_conditional_market_eligibility.py`, `test_agent_overall_reconciliation.py`, `test_agent_target_odds.py`).

**Cache-busting side effect, not a separate fix:** `min_conditional_odds_threshold` being a genuinely new required `AgentConfig` field means `compute_agent_config_hash()` changes for every config on deploy — every existing `RecommendationCache` row's key stops matching, so the next EOD/T-30 pass finds `already_fresh()` false everywhere and regenerates fresh. This happens to close A65's own "already-cached rows keep their stale `overall`" limitation (Section 27.3 above) without a manual `/api/admin/pregenerate-recommendations` trigger — a byproduct of this specific fix shape, not a general mechanism to rely on for future logic changes that don't happen to add a new config field.

Full story-level detail lives in `documents/agent_user_stories.md` A66 and `documents/app_user_stories.md` W154 (the paired frontend guard — `current_odds > 0`, not just non-null, for the deploy window before the cache-busting regeneration above catches up).

### 27.5 Value-edge coherence — `_downgrade_direct_bet_below_value_edge_floor()` (`src/agent/schema.py`, A67)

Same investigation, direct follow-up: a live card showed `direct_bet` with `EDGE -13.8%` — a market claiming to be a direct bet while its own `value_edge` was strongly negative, and ranking in the "Top Edges" rail alongside genuine positive-edge picks. Root cause: A29 (odds bounds), A54 (market eligibility), and A66 (the conditional floor) all code-enforce that `recommendation_type` matches the market's own *price* data — none of them ever checked `value_edge` itself against `min_value_edge`, the one field that actually defines "does this market have real value" in the first place. The LLM's own arithmetic (`value_edge = ml_probability - implied_probability`) was trusted uncritically for the *classification* decision, same class of gap A65 closed for `overall` and A66 closed for `conditional`'s price floor, just for `direct_bet`'s own defining claim.

`_downgrade_direct_bet_below_value_edge_floor(data, min_value_edge)` — no new config parameter; reuses `extract_recommendation()`'s existing `min_value_edge` (already threaded through for `_compute_target_odds`, Section 27.3-adjacent). Runs **first** in the downgrade sequence, before A29's own bounds check: a market with no genuine underlying value (negative or sub-floor `value_edge`) must never reach A29's "reclassify as conditional" path at all — there's nothing to wait for at any price if the model's own probability doesn't support it. Downgrades straight to `no_bet`, same BUG-013 precedent. Deliberately scoped to `direct_bet` only: a `conditional` market's entire premise is "not enough edge *yet*, wait for a better price" — a low or negative current `value_edge` there is expected, not incoherent, and untouched by this pass.

No frontend change needed — confirms the "one field, many consumers" architecture (A65/W153) is paying off as designed: `bestMarket()`'s edge ranking, `pricedEdge()`'s `no_bet`-exclusion (the "Top Edges" rail, `dashboardMetrics.ts`), and A65's `overall` reconciliation all automatically stop surfacing a market as an edge once `recommendation_type` itself is correct. The bug was purely in what the backend labeled the market; every consumer reading that label was already correct.

Full story-level detail lives in `documents/agent_user_stories.md` A67.

## 28. Secondary-Market Odds Coverage — `total_goals_odds` Threading and a Permanent BTTS/Corners Gap (A69, 2026-08-21)

Found live while reviewing SP1's first `agent-train` lesson batch (299 real lesson candidates, direct user request): 209/299 (70%) flagged "no current odds" for a secondary market (btts, total_goals, or corners), even though these matches have a real, recorded `result_3way` price. Investigated why, market by market:

**`total_goals` (over/under 2.5) — real data existed but was never wired through. Fixed here.** `raw_matches` has genuinely real, populated `over25_odds`/`under25_odds` columns (sourced from football-data.co.uk's own `Avg>2.5`/`Avg<2.5` CSV columns — confirmed via `src/ingestion/football_data/loader.py`), but `_build_match_info()` (`src/agent/backtest.py`) — the function every `agent-train`/`agent-backtest` match's input passes through — only ever extracted the 3-way odds (`odds_h`/`odds_d`/`odds_a`). This affected every league's backtest/training run identically, not just SP1.

Fixed by adding a `total_goals_odds` key (`{"over_2.5": ..., "under_2.5": ...}`), gated on `pd.notna()` on both sides rather than plain truthiness — a real DataFrame's missing value is `NaN`, and `bool(float("nan"))` is `True` in Python, so a naive `if row.get(...):` check (the pre-existing pattern for the 3-way odds above it, left unchanged since it's a separate, untouched concern) would have silently let a NaN "price" through. Populating `match_info` alone would have been a no-op, though — `run_agent()`'s own prompt-building (`src/agent/graph.py`) only ever mentioned `match_info["odds"]` (3-way); a new `if total_goals_odds:` clause appends a matching sentence (`"Bookmaker odds for total goals (over/under 2.5): over_2.5=X, under_2.5=Y."`) so the LLM actually receives the price, not just the pipeline. 6 new tests: `tests/test_backtest.py` (`_build_match_info`'s 3-way-unchanged, both-sides-present, absent-from-row, one-side-only, and NaN cases) and `tests/test_agent_graph.py` (`run_agent`'s prompt actually contains the new sentence, inspected via the mocked LLM's own call args — not just that `match_info` carries the key).

**`btts` and `home_corners`/`away_corners` — no fix possible; no data source anywhere in this system carries these odds, live or historical.** Checked every currently-integrated ingestion source: `src/ingestion/football_data/` (the historical CSV — only 1X2, over/under 2.5, and Asian handicap; grep for "btts"/"corner" across the whole ingestion tree returns zero hits outside this section), Understat (xG only, no odds fields at all), FotMob (player/lineup stats only, no odds fields at all), football-data.org (fixture/result metadata only, no odds fields at all). Also checked the *live* path (`app/backend/odds_api_client.py`): The Odds API integration is deliberately scoped to `markets=("h2h",)` only (their cost model is `markets × regions` per call) — so live serving doesn't fetch `totals`/`btts`/anything else either, even though The Odds API's live product does support a `totals` market and BTTS as an "additional market" at extra cost. Net effect: **live and backtest are currently equally blind to btts/corners odds**, for different underlying reasons (no data source at all vs. a data source that isn't being called for that market).

**If a new market's odds ever need wiring up (user's own stated intent — checklist for that work):**
1. Does `raw_matches` already have a real column for it? (Check `information_schema.columns` for the target table, and `src/ingestion/football_data/loader.py`/`match_schema.py` for what football-data.co.uk actually publishes — currently only 1X2, O/U 2.5, and Asian handicap.) If yes, thread it through `_build_match_info()` (`src/agent/backtest.py`) the same way this story did for `total_goals_odds`, **and** add the matching sentence to `run_agent()`'s prompt-building (`src/agent/graph.py`) — populating `match_info` alone is not sufficient, confirmed the hard way by writing the prompt-content test above.
2. If no historical column exists, does The Odds API's live feed cover it? Adding it means widening `OddsAPIClient`'s `markets` tuple (`app/backend/odds_api_client.py`) — a real, separate cost increase (`markets × regions` per call), and doesn't help backtesting at all, since `historical_odds_client.py`'s own docstring already documents that The Odds API "is a live-current-odds-only service with no historical replay."
3. If neither existing source has it (true today for btts/corners), a new vendor integration is required. Live-tested one real candidate during this investigation, [OddsPapi](https://oddspapi.io/us) (real API key, real calls, not just their marketing page): its live feed genuinely does carry BTTS (`marketId 104` in their own `/v4/markets` catalog) and extensive corners market variants for real E0/SP1 fixtures via both Pinnacle and bet365 (confirmed: 39-41 BTTS instances, 345-1764 corners instances across ~23-26 live fixtures pulled). **Unresolved and the actual blocker for backtesting use**: their `/v4/historical-odds` endpoint, tested against a real fixture, returned price-movement snapshots starting only ~2 weeks before that match's kickoff (line movement for an *upcoming* game), not an archive reachable for an already-completed match from a prior season — no fixture-ID lookup path for a completed historical match was found in the time spent, so genuine season-back historical depth for this or any other candidate (TheStatsAPI, OddAlerts, Goalserve, Football Corners API) remains unverified. Any future integration work should test this specific gap first (a completed match's fixture ID against the historical endpoint) before committing to a vendor.

Full story-level detail lives in `documents/agent_user_stories.md` A69.

## 29. Provider Decline Bias, a Live Default Switch, and a Stop-on-Repeated-Data-Issues Rule (A70–A73, 2026-08-22)

Root-caused why SP1's bet rate stayed near zero even after A67's coherence bullets were confirmed working as intended: DeepSeek cites the forecast's own `uncertainty`/`entropy` diagnostic (a real field, `normalized_entropy_uncertainty()` in `src/forecast/uncertainty.py`, always attached to every classification-market forecast and serialized verbatim into evidence by `_format_evidence_message()`) as a ready-made reason to decline a qualifying `value_edge`, regardless of prompt wording — confirmed across 6 distinct prompt variants. Added `AgentConfig.suppress_forecast_uncertainty` (A70) to strip that field from evidence; it worked exactly as designed (zero entropy mentions afterward) but the model substituted a different, sometimes factually false, decline reason instead of accepting the edge — evidence the bias is model-level, not prompt- or evidence-shape-dependent.

Confirmed via a controlled cross-provider test (A71): holding the exact same config/prompt/evidence/window fixed and swapping only the provider, Claude Haiku 4.5, Gemini 3.6 Flash, and Groq (`openai/gpt-oss-120b`/`gpt-oss-20b`/`qwen3.6-27b`, confirmed model-independent) all accept edges DeepSeek declines. User enabled billing on Gemini and asked to fully switch — `config/agent_config.yaml` (the live default; `app/backend/main.py` calls `AgentConfig.default()` directly, so this reaches production once merged) now points at `gemini-3.6-flash`/gemini (A72). `suppress_forecast_uncertainty` stays off on the live default — it exists to route around DeepSeek's bias specifically, and Gemini doesn't exhibit it.

**A73 — a process failure, not just a data bug.** Built `.claude/skills/agent-train-experiment` (stratified `--sample`, `--batch-size 1`, a combined per-match experiment log, a "popular lessons" summary) and ran it once for real on SP1 (100-sample). 96/99 lessons complained about missing `total_goals` odds — a full day after A69 supposedly fixed exactly that. Root cause: `_build_match_info()` was correct in isolation (all 6 of A69's own unit tests passed), but `BacktestHarness.load_matches()`'s SQL `SELECT` — the actual data loader every real run goes through — never fetched `over25_odds`/`under25_odds` from `raw_matches` at all, so `row.get(...)` always returned `None` in production. A69 looked complete because nothing tested the real caller, only the function.

**The generalizable lesson, not just the specific bug:** a whole 99-match run (and real API spend) completed before anyone noticed the systemic pattern in the data — the fix only happened because the user asked "why is this still happening?" after seeing the popular-lessons summary. That's backwards: a lesson-generation run repeatedly flagging the *same* data-quality complaint across a large fraction of matches is itself a signal worth stopping on, before spending more budget or drawing conclusions from a corpus that's quietly degraded. This is now an explicit rule in `.claude/skills/agent-train-experiment/SKILL.md`'s Steps 6/7: if a majority of a run's lessons converge on one recurring data-issue theme, stop before running further samples/postures on the same config, and treat it as a probable code/data bug to root-cause (per `systematic-debugging`), not just another "popular lesson" to report and move past.

Full story-level detail lives in `documents/agent_user_stories.md` A70–A73.

## 30. Provider Vendor Survey, a Real Timeout/Rebilling Bug, and Cross-League Lesson Generalization (A74–A78, 2026-08-22/23)

Following A71/A72's provider switch, this phase surveyed further alternatives (curiosity-driven, all direct user requests) and hit one genuine, severe infrastructure bug along the way. Summary table first, mechanism and API-issue detail after.

| Provider/model | Outcome | Why |
|---|---|---|
| `deepseek-chat` (V4-Flash, the pre-A72 default) | **Confirmed unusable for this task** | Decline bias (A71) — 0 bets across 6 prompt variants, the A70 fix, and (A77) a clean held-out test-split re-check after this session's lesson updates. Cheapest per-token, functionally broken regardless. |
| `gemini-3.6-flash` (live default since A72) | **Working, validated default** | Accepts qualifying edges; positive ROI on 4/5 leagues in real 30-sample checks; clean train/test-split lesson methodology confirmed it holds up. Mid-priced ($0.75/$3.75 per M in/out). |
| `claude-haiku-4-5` | Confirmed to avoid the decline bias (A71) | Not pursued further — cost, not behavior, ruled it out. |
| Groq (`openai/gpt-oss-120b` etc.) | Confirmed to avoid the decline bias (A71); later reused for a cheap admin task | Free/cheap tier too rate-limited (8,000 tokens/min account-wide) for calibration-scale volume; still useful for the A76 lesson-approval conflict-check once both Gemini and DeepSeek ran out of funds mid-task — that's a cheap text-comparison job, not a betting decision, so DeepSeek's known bias doesn't even apply there. |
| `qwen3.8-max` (QwenCloud, A74) | **Tried, dropped** | Architecturally clean addition (OpenAI-compatible, same `ChatOpenAI` pattern as `deepseek`) and behaviorally fine (accepted qualifying edges), but real cost came in ~10x pre-test estimates *twice* — first on the killed 58-match comparison, confirmed again reconciling QwenCloud's own usage dashboard (A75 follow-up). Root cause below. Also pricier per-token than Gemini or DeepSeek regardless. |
| `deepseek-v4-pro` | **Genuinely promising, still under-sampled** | A different, pricier DeepSeek model than the broken default — real 5-league test-split run (A77, 73 matches, ¥8.68 ≈ $1.29) landed 4/5 leagues positive, the strongest real result of any provider tested this session. Not promoted anywhere; needs a larger sample before treating as more than "worth taking seriously." |
| Apple Private Cloud Compute | **Researched, ruled out** (A78) | Real WWDC 2026 announcement, verified live rather than assumed. Free tier is gated to the App Store Small Business Program (FPAI isn't an App Store app); even if eligible, access is via an on-device Swift framework, not a server-callable REST endpoint — doesn't fit `_build_llm()`'s architecture at all. No code involved. |

**The API issue (A75), in full — worth its own explanation since it's a generalizable infrastructure gap, not a Qwen quirk.** `ChatOpenAI`'s `request_timeout` field (the real name behind the `timeout=` kwarg) defaults to `None` — no timeout at all. `_invoke_with_retry()` (`src/agent/graph.py`) only retries on a *raised* exception; a request that never returns also never raises one, so a single bad connection can block a worker thread indefinitely. Found live: a real qwen3.8-max backtest appeared stalled for ~9h46m elapsed wall-clock time with only 8 seconds of CPU used. Diagnosed systematically before touching anything — `lsof` showed one `ESTABLISHED` TCP connection with zero data flow (a stuck read, not a crash); killing the process and reading its *full* captured output (not just the tail, which had already discarded the useful part once already this same investigation) showed it had genuinely processed 34/58 matches normally in ~14 minutes, then hit real `Connection error`s that each took an unbounded, sometimes multi-hour amount of time to actually surface as a catchable exception.

Fixed with an explicit 90-second timeout (`_OPENAI_COMPATIBLE_TIMEOUT_SECONDS`) on both OpenAI-compatible branches (`deepseek`, `qwen` — same underlying gap, same fix), sized from real observed per-match latency with margin. This caps the *time* wasted on a hung connection, but a second, distinct problem surfaced when reconciling the real dollar cost afterward: QwenCloud's own usage dashboard logged 165 "100% successful" requests against only ~46 actually-intended match evaluations, at 128.8s average latency — strong evidence that many of the client-side "Connection error" retries were requests QwenCloud's server had *already fully processed and billed*, whose responses simply never made it back to the client (a network-level failure, not a server rejection). Our own retry logic then resent the entire large context from scratch and got billed again. No idempotency-key or request-status mechanism was found in QwenCloud's docs to prevent this — the client-side timeout reduces *how long* this can drag on, but doesn't prevent the rebilling itself. A generalizable fix (exponential backoff + a batch-level circuit-breaker that stops the whole run after N consecutive failures instead of grinding through hours of retries) was identified and offered to the user but not yet built — it would reduce exposure on *any* OpenAI-compatible provider going through `_invoke_with_retry`, not just Qwen.

**Cross-league lesson generalization (A76):** after the SP1-derived "draw-framing fallacy" lesson (agents citing "evenly matched"/"tightly contested" framing as if it were independent evidence for a draw pick, when it doesn't correlate with actual outcomes) reproduced independently across all five leagues tested (23-38% draw hit rate each, 28% aggregate across 85 picks), it was promoted from a per-league (`scope="competition"`) rule to a single tier-wide (`scope="tier"`) rule — all five leagues share the identical `tier` value (`competition_specific`), confirmed via a direct query before relying on it. A second candidate lesson (result_3way edges are less reliable than total_goals edges) held on only 3/5 leagues (SP1, E0, I1 — not D1/F1) and was deliberately kept competition-scoped, with each rule's own text noting the negative result on the other two leagues so a future reviewer sees it. This work also caught a real process gap: E0 already had a genuine, human-approved lesson (2026-07-28) that wasn't checked for before a new insert attempt, unlike SP1 where the zero-existing-rules baseline was explicitly verified first — the A45 conflict-check caught it as designed.

Full story-level detail (real ROI numbers per league, exact cost reconciliation, dashboard screenshots referenced, the CNY/USD conversion) lives in `documents/agent_user_stories.md` A74–A78.

## 31. Tighter Live Production Odds Range, and a New Conditional Ceiling (A84, 2026-08-28)

Direct user request: narrow the live production config's recommendable odds range to -140/+300 American (decimal `[1.71, 4.0]`), tighter than A29's existing `[1.2, 11.0]` band. Two design decisions, both confirmed with the user before implementation: (1) production config only — `config/agent_config.yaml` changes, every posture (conservative/balanced/aggressive), the DeepSeek variant, and the backtest config keep A29/A66's original, wider band; (2) `conditional` recommendations are capped to the same band too, not just `direct_bet`.

(2) exposed a real, pre-existing structural gap worth calling out on its own: A66 (Section 27.4) gave `conditional` a *floor* (`min_conditional_odds_threshold`) but never a *ceiling*. Since A29's own downgrade path sends an out-of-band `direct_bet` to `conditional` regardless of which side it's out of band on (too short *or* too long), a `direct_bet` priced above `max_odds_threshold` had no upper bound at all once it became `conditional` — it would sail through unbounded. This was invisible before because `max_odds_threshold` (11.0) was wide enough that nothing realistically exceeded it in practice; tightening `max_odds_threshold` to 4.0 made the gap immediately reachable.

New `AgentConfig.max_conditional_odds_threshold` (`src/agent/agent_config.py`), added the same way `min_conditional_odds_threshold` (A66) already is, with one difference: **optional**, defaulting to `float('inf')` rather than a new required field — every config that doesn't explicitly set it (every posture/DeepSeek/backtest config today) keeps today's real, pre-existing unbounded-ceiling behavior with zero changes to its own yaml. Only `config/agent_config.yaml` sets a real value (`4.0`). New `_downgrade_conditional_above_ceiling(data, max_conditional_odds_threshold)` (`src/agent/schema.py`) mirrors `_downgrade_conditional_below_floor()` (A66) symmetrically — a `conditional` market's `current_odds` above the ceiling (null out of scope, same precedent) downgrades to `no_bet` — and runs immediately after it in `extract_recommendation()`'s fixed pipeline, before A52's `target_odds` computation. Threaded through `_build_recommendation()` (`src/agent/graph.py`) and `compute_agent_config_hash()` (Section 6.3 of `app_techspec.md`), same pattern as the other three threshold fields.

**Reversed invariant, by explicit user choice:** `docs/superpowers/specs/2026-08-21-agent-risk-posture-design.md` had deliberately kept `min_odds_threshold`/`max_odds_threshold`/`min_conditional_odds_threshold` fixed across every posture variant, framed as "safety rails, not an aggressiveness dial" — enforced by a test asserting all three equal the production default. This story breaks that equality for the production config specifically; the test (`tests/test_agent_config.py::test_all_three_posture_configs_keep_every_other_field_identical_to_default`) was updated to drop the odds-threshold equality assertions (model/temperature/max_tool_calls/markets are still asserted equal), with two new tests added to lock in the new, deliberately-divergent shape: `test_default_config_has_2026_08_28_odds_range` (production's new exact values) and `test_posture_and_backtest_configs_keep_the_original_a29_a66_odds_bounds` (every other config still at the original values).

**Cache-busting side effect, not a separate fix** (same mechanism as A66, Section 27.4): `max_conditional_odds_threshold` joining `compute_agent_config_hash()`'s payload, combined with `min_odds_threshold`/`max_odds_threshold`/`min_conditional_odds_threshold`'s changed values, means every cached recommendation's config hash changes on this deploy — the next EOD/T-30 pass regenerates fresh rather than reusing a stale-hashed cache row.

Full story-level detail lives in `documents/agent_user_stories.md` A84.

## 32. Tavily Secondary Fallback Key & Tool-Call Budget Cut (A86–A87, 2026-08-29/30)

**A86 — `TAVILY_API_KEY_FALLBACK`.** `_web_search_impl()` (Section 4.1) builds an ordered, deduped list from `os.environ.get("TAVILY_API_KEY")` and `os.environ.get("TAVILY_API_KEY_FALLBACK")`, trying each in turn on the previous key's exception, keeping A53's per-attempt warning log — degrades to the same `TOOL_PERMANENTLY_UNAVAILABLE` sentinel only once every configured key has failed. Single call site, single env var, no config/schema changes.

**A87 — production `max_tool_calls` cut 10 → 3, one day later, after the second Tavily key also hit its own plan's usage limit.** Root-caused via `superpowers:systematic-debugging`, not guessed: the two-log-lines-per-query evidence (both keys attempted, both rejected with the same quota message) confirmed A86's fallback logic was executing exactly as designed — the second Tavily account had independently exhausted its own quota, unrelated to A86's own correctness. Tracing the real driver found `research_node`'s (`src/agent/pipeline.py`) 2 guaranteed baseline searches run on *every* `run_agent()` call, including each T-30 refresh (`app/backend/t30_refresh.py`) — not just the first recommendation per match — so quota burn scales with refresh frequency, not distinct matches. `max_tool_calls` never bounded that floor at all; it only bounds the agent's own discretionary *extra* `web_search` calls beyond it. Cut in `config/agent_config.yaml` only (production), `ponytail:`-flagged as an untuned guess pending real `tool_call_count` usage data — posture configs, `agent_config_deepseek.yaml`, and `agent_backtest_config.yaml` all left at 10, same production-only scoping precedent as A84.
...
## 35. Live-Wait Conditional Strategy — `_promote_favorite_to_conditional_for_live_wait()` (A112, 2026-09-17)

Direct user strategy: wait into a match for an early non-event (concrete example — `btts` priced around −150, wait ~20 minutes, if neither side has scored yet the live price reliably drifts out toward +100 with the true probability barely changed) to get a better number on a pick that's still likely to win. Distinct in kind from every other `conditional` rule in Section 27.3–27.5: those react to a *pre-match* price sitting outside a comfort zone; this one is about waiting for an *in-match* event (or non-event) to move the price — the app has no live/in-play odds feed today, so this is scoped as a pre-match labeling rule only (flags good candidates for the strategy using data already available), not real live-odds tracking, per direct user agreement that live tracking is a future stretch goal.

**Mechanics** (`_promote_favorite_to_conditional_for_live_wait()`, `src/agent/schema.py`). Unlike every downgrade pass in this file, this one *promotes*: it turns `no_bet` into `conditional` — a gap the value-edge check alone wouldn't fill, since the premise is the current price/edge doesn't need to clear the bar yet, only the price this is expected to drift to needs to. A candidate qualifies when all of:

1. `recommendation_type == 'no_bet'` — scoped to `no_bet` only (see 35.1 below for why an already-qualifying `direct_bet` is deliberately never touched here).
2. `(market, selection)` is in the same `_CONDITIONAL_ELIGIBLE_MARKETS` set A54 already restricts `conditional` to (`btts/yes`, `total_goals/over_2.5`, the three corners-over markets) — waiting has to be a real directional strategy, not a coin flip.
3. `current_odds >= live_wait_min_odds` — new `AgentConfig.live_wait_min_odds: float | None = None` (opt-in; only `config/agent_config.yaml`, the live production default, sets a real value — `1.6667` decimal / −150 American, the user's own stated floor: "anything over −150 is good enough"). Deliberately *looser* than the general `conditional` floor A66 already enforces (prod default `1.71`/−140) — this rule runs after A66's own pass so a newly-promoted −150 pick isn't immediately undone by a floor built for a different (pre-match-drift-only) premise.
4. `current_odds < live_wait_target_odds` — new `AgentConfig.live_wait_target_odds: float = 2.0` (decimal / +100 American, the price this strategy assumes it's waiting for) — mostly a defensive/structural guard for a `no_bet` candidate (one already priced longer than the target would have cleared edge at the target too, by the same monotonic-implied-probability reasoning), not the load-bearing check it was in the original design (see 35.1).
5. `ml_probability - implied(live_wait_target_odds) >= min_value_edge` — direct user refinement, same day: *"when there is no direct bet, a conditional bet can be recommended if the target odd (+100) clears the edge requirement."* Replaces an original bare `ml_probability > 0.5` check.

Runs in `extract_recommendation()`'s pipeline after A66's floor check and the ceiling check (ordering reason in check 3 above), before A52's `target_odds` computation — so a newly-promoted candidate gets a real target price computed for it too, same as any other `conditional` pick. `live_wait_min_odds=None` (the default for every posture config except the live production one) disables the whole rule; every other caller/test is unaffected by construction.

### 35.1 Redesign: the `direct_bet` Trade-off Is a Cross-Candidate Decision, Not a Same-Candidate One (same day, 2026-09-17)

The first version of this rule also overrode an already-qualifying `direct_bet` to `conditional` on the *same* candidate, whenever it separately met the price/probability checks above. Found wrong via a real, user-shared card: Espanyol v Elche's `btts/yes`, the only qualifying candidate on the match, real edge 9.4% at 1.73 (−137) — a genuine, already-clearing direct bet. The rule relabeled it `conditional` anyway, and the user asked the obvious question: *"if the edge is already 9.4%, why would I wait?"* Two real problems this surfaced: (1) throwing away a **certain** edge in hand for an **uncertain** future one, with no comparison of which is actually the better trade, and no accounting for the real risk that the price moves the *wrong* way (an early goal makes `btts:yes` more likely, not less — the price would shorten, not drift to +100); (2) the LLM's own explanation bullets are written *before* this override runs, so the card's narrative still read like a direct-bet recommendation ("the bet can be taken at the current price rather than waiting") while the badge said "Conditional" — a live, user-visible contradiction, structurally the same class of badge/content mismatch A65 (Section 27.3) was built to fix for `overall`, but for per-market explanation text, which A65 never touches.

Direct user clarification once this was raised: overriding an already-qualifying `direct_bet` is a **cross-candidate** decision — comparing it against a genuinely *different*, higher-probability candidate that only qualifies once its own price improves — not a same-candidate relabel. Concrete example given: a match with a `draw` `direct_bet` (10% edge, 30% `ml_probability`) and a `btts` candidate that only qualifies as `conditional` once its price hits the target (4% edge now, would be 5% at +100, 55% `ml_probability`) — the `btts` conditional pick is the *better* recommendation, because its win probability is far higher, even though its edge is smaller and it isn't bettable at the current price at all.

**New `_prefer_higher_probability_conditional_pick()`** (`src/agent/schema.py`), a second, separate function. Only acts when `recommendation_pick` currently resolves to a `direct_bet` candidate; finds every *other* candidate that is `conditional` and in `_CONDITIONAL_ELIGIBLE_MARKETS`, and switches `recommendation_pick` to whichever has the single highest `ml_probability` — but only if that beats the current `direct_bet`'s own probability. A no-op in every other case: no eligible conditional alternative exists (heals the original Espanyol/Elche card — a uniquely-qualifying `direct_bet` now correctly stays one), the best alternative isn't actually more probable, the candidate is in an ineligible market, or the resolved pick isn't `direct_bet` to begin with. Runs after A91 (`_downgrade_recommendation_below_top_composite_score`) — that check validates the LLM's *own* self-consistency against its own pick, which would be the wrong thing to re-run against a pick this function itself just chose — and before `_resolve_recommendation_pick`, so the switch propagates to `overall` (and therefore the frontend badge) through the exact same existing sync mechanism, no separate logic needed.

**Verification.** `tests/test_agent_live_wait_conditional.py` (20 tests, rewritten: the `no_bet`-promotion tests from the original design carry over largely unchanged in spirit, plus a `direct_bet`-candidates-are-never-touched regression test replacing the old override test, plus 7 new tests for the pick-switching function — the exact draw/btts scenario, no-switch when the conditional isn't more probable, no-switch when no eligible alternative exists, no-switch for an ineligible market, no-switch when the pick isn't a `direct_bet`, and the two explanation-rebuild tests below), `tests/test_agent_config.py` (+2: default config's real values, posture configs stay `None`). Threaded through `graph.py`'s `_build_recommendation()` call site.

**Follow-up, same day: `explanation` staleness.** `explanation` (`list[str]`, A88's bullet-point redesign) is the LLM's own prose, written to justify its *original* pick — left untouched after a switch, it kept reading like a case for the candidate no longer shown (e.g. draw-specific head-to-head reasoning while the badge/market now displayed is `btts`), the exact narrative-vs-badge mismatch class this whole redesign started from, just one layer deeper (the badge/`overall` sync was already correct via `_resolve_recommendation_pick`; the free-text `explanation` had no equivalent sync at all). Fixed in the same function: on a switch, `data["explanation"]` is replaced with a switch-rationale bullet (the same text appended to `limitations`) plus the new pick's own one-line self-reported `reason` field (every `MarketCandidate` already carries one, A88 — no new LLM call or synthesis needed). Left untouched whenever no switch happens. 2 new tests. Full suite: 1086 passed, 1 skipped, zero regressions.
...
Direct user spec for a redesigned recommendation-detail UI: icon-tagged blocks (value case, per-side team evidence, a judgment "read", betting-price note) instead of `explanation`'s flat bullet list, with strict content rules -- ground every claim in match facts, never name a threshold/price range/internal cutoff ("a person should only be able to disagree with the read, not the logic"), show the model's own probability as data (a chart) rather than narrating it in prose. Frontend half tracked as W226 (`app_user_stories.md`/`app_techspec.md` §15.17, same investigation) -- the two blocks that need no LLM involvement at all (value case, betting price) are built entirely there from data already on the resolved pick.

**New additive `MatchRecommendation` fields**, populated by `normalize_structured_reasoning()` (`src/agent/schema.py`):

- `team_evidence: {"home": str, "away": str} | None` -- exactly one plain match fact per side (direct_bet/conditional only). No probabilities, no odds, no "edge"/"value"/threshold language -- those render as data elsewhere, not prose.
- `the_read: str | None` -- one sentence of judgment on why the pick makes sense, grounded in the two `team_evidence` facts (direct_bet/conditional only). Never compares to a runner-up market, never cites an edge number or any internal cutoff.
- `no_bet_read: str | None` -- the same judgment, for the `no_bet`/`insufficient_data` case. Same restriction, plus: never states how close anything came to qualifying.

**Lenient by design, never strict.** These are additive, not a replacement -- `explanation`/`limitations` are populated exactly as they always have been regardless, so a client that doesn't render the new fields (or a pre-A113 cached row, or a model response that doesn't comply with the updated prompt) still gets everything it always had. `normalize_structured_reasoning()` never raises: a malformed or absent value degrades to `None`, the frontend's own signal to fall back to the flat-list rendering for that section. Deliberately **not** added to `MatchRecommendationModel`'s strict Pydantic validation (used both for `extract_recommendation()`'s own `model_validate()` gate on parsed JSON, and directly as the schema bound to ollama's `_structured_output()`, §33/A37) -- adding strict typing there would let one malformed new field fail parsing of an otherwise-valid recommendation on the production DeepSeek path. Consequence, disclosed rather than silently accepted: ollama's schema-constrained structured-output path can't produce these fields at all (nothing in its bound schema allows it), a real but low-priority gap since ollama isn't the production provider (§29/A70's default-provider switch).

Threaded through `app/backend/recommendations.py`'s `validate_and_degrade()` -- the actual API-serving layer, independent of `extract_recommendation()` -- the identical lenient way: `normalize_structured_reasoning(dict(raw))` (a copy, since `normalize_structured_reasoning` mutates its argument and `raw` is the caller's own dict, not this function's to modify as a side effect), with `MatchRecommendationOut`'s new `team_evidence`/`the_read`/`no_bet_read` fields all optional-defaulting-to-`None`, matching `unit_bet_multiplier`'s own precedent for a pre-this-change cached row.

**Same staleness class A112's own `explanation` fix (§35.1) already covers, closed here too.** `_prefer_higher_probability_conditional_pick()` clears `team_evidence`/`the_read` to `None` on a pick switch, rather than leave content written to justify the *original* candidate sitting under a different one now shown -- there's no per-candidate replacement text for them the way `reason` covers `explanation`, so they're cleared rather than guessed at; the section falls back to the (already-rebuilt) flat list for that card, not a broken or missing one.

Prompt updated identically across all 4 files (`agent_v1.txt` + 3 posture variants) -- both a new instructional paragraph (the per-field content rules above) and three new keys in the JSON schema block, inserted right after `explanation`.

**Verification.** `tests/test_agent_structured_reasoning.py` (7 tests: valid pass-through, fields-absent-entirely degrades gracefully rather than failing parsing, a partial `team_evidence` [one side only] degrades to `None`, a wrong-typed `team_evidence` degrades to `None`, a blank `the_read` degrades to `None`, `no_bet_read` passes through for the `no_bet` case, and the pick-switch clearing case), `app/backend/tests/test_recommendations_schema.py` (+3: pass-through, missing-defaults-to-None, malformed-degrades-not-crashes). Full suite: 1093 passed, 1 skipped; backend suite: 564 passed (5 pre-existing unrelated `test_fixtures_endpoint.py` failures, unrelated to this change). Zero regressions.

Full record: `documents/agent_user_stories.md` A113, `documents/app_user_stories.md` W226.

## 37. `explanation`/`limitations` Get the Same Content Rule (A114, 2026-09-17)

A113's strict "no threshold/edge/internal-cutoff" content rule (§36) only ever applied to the three new fields (`team_evidence`/`the_read`/`no_bet_read`) -- its own completion note said so explicitly ("`explanation`/`limitations` are populated exactly as before regardless"). Direct user report on a live card exposed the gap this left open: W231's fallback rendering (`app_user_stories.md`) puts `explanation` inside the same "The read" row `the_read` occupies, so a card without the new fields showed exactly the language A113 was written to ban -- "BTTS 'no' is the only priced market that clears the 5% value bar... equals a +10.6% edge" -- and the raw `limitations` list (rendered separately, W232) surfaced model-internal stats verbatim: "The BTTS classifier has near-coin-flip held-out log_loss (0.689) and high entropy, so its 68.4% 'no' probability may be overconfident."

**Prompt-only fix, all 4 files (`agent_v1.txt` + 3 posture variants), identical edit:**

- `explanation`'s instruction paragraph no longer invites "the value-edge math for that market," nor uses "no market's value edge exceeded the 5% threshold" as its `no_bet`-case example -- both replaced with plain-match-terms phrasing. It now carries the same restriction A113 gave `team_evidence`/`the_read`: no probabilities, no odds, no value-edge math, never say "edge"/"value," never name a threshold, price, or other internal cutoff.
- That restriction is explicitly extended to `limitations` in the same sentence -- plain match facts about what's uncertain (a doubtful injury, unclear team news, a thin recent-form sample), never a model-internal caveat (log_loss, entropy, calibration, confidence stats), and never a restatement of `explanation`'s own value-edge math in a different sentence.
- The existing instruction to consult each forecast target's own held-out `metrics` (§7/line 11, log_loss/accuracy/mae/rmse -- "use it to gauge how much to trust that specific target's prediction") is now scoped explicitly as internal reasoning only: those metric values, and the phrase "held-out," must never surface in `explanation`, `limitations`, or any Why-This-Pick field. A target's low reliability can still affect confidence or the pick itself -- it just has to be stated in plain terms, not the underlying stat.

No schema, field, or code change -- `MatchRecommendation.explanation`/`limitations` (`src/agent/schema.py`) keep their existing `list[str]` shape and validation exactly as A113 left them; this is purely a content-shape instruction to the model. Frontend half (removing the raw un-sectioned `limitations` `<ul>` that bypassed the icon-row system entirely, plus the same-day row-highlight gradient/contrast fixes) tracked as W232 (`app_user_stories.md`/`app_techspec.md` §15.22).

Full record: `documents/agent_user_stories.md` A114.

## 38. Market-Prioritized Research Strategy and Injury Recency (A115-A116, 2026-09-18)

Direct user spec (2026-09-17): `research_node`'s evidence was market-agnostic -- one fixed injury/team-news search plus one recent-form search fed a single LLM pass reasoning about every market (`result_3way`/`btts`/`total_goals`/corners) at once, with no notion that different markets should weigh different evidence more heavily. User specified per-market priority tiers (e.g. `result_3way` should weigh match context/motivation above squad depth; `btts` should weigh defensive-vs-scoring-consistency above game-state tendencies) and separately flagged that a recent injury can contradict `FORM_SEARCH_RESULT` (the injured player was likely part of the form sample) while a long-standing one is probably already priced into it.

**Search-budget decision (direct user choice among three options presented):** a literal per-market-per-tier search fan-out was rejected as too costly against §32's ~$0.02/match calibration and Tavily's shared-account quota (A86/A87). Chose instead to consolidate the new evidence into one additional broad query rather than one query per market.

**A115 implementation, `research_node` (`src/agent/pipeline.py`):** added a third guaranteed search, `"{home} {away} match preview tactical analysis league position motivation"`, run every call (near-kickoff/regular alike) after availability and before recent-form. Stored as `research_evidence["match_context"]`, surfaced to the LLM as a new `MATCH_CONTEXT_SEARCH_RESULT:` line in `_format_evidence_message`. Guaranteed-search count per match: 3 baseline (was 2) + conditional odds-verification + conditional near-kickoff fallback, unchanged pattern.

Prompt-side (`config/prompts/agent_v1.txt` + 3 posture variants, identical edit): new "## Evidence Priority by Market" section lists each market's priority-ordered evidence (result_3way: motivation → tactical/home-away profile → squad depth; total_goals: tactical style/conversion → both-ends absences → pitch conditions; btts: defensive-vs-scoring split → defensive/GK absences & set pieces → game-state tendencies; corners: wing-play/game-state, single tier) and states that on conflicting evidence, the higher tier wins the read rather than an averaged judgment. This is a reasoning-weight instruction, not a new schema field or code-enforced rule -- no acceptance check beyond prompt content exists yet.

**A116 implementation:** folded into the same prompt section rather than a separate query -- a new paragraph instructs the LLM to distinguish a recent injury (contradicts `FORM_SEARCH_RESULT`, treat as new information) from a long-standing one (likely already reflected in recent form, don't double-count) when reading `AVAILABILITY_SEARCH_RESULT`. No new search or field: the existing injury query's results already carry recency language (dates, "doubtful for Saturday" vs. "out for the season") for the model to read.

Tests: `tests/test_agent_pipeline.py` -- all `research_node` search-sequencing tests updated for the third call (`test_research_node_runs_availability_context_and_form_searches_only_when_odds_supplied`, odds/near-kickoff/fallback variants), plus a new assertion that `_format_evidence_message` emits `MATCH_CONTEXT_SEARCH_RESULT`. Full suite: 1740 passed, 1 skipped, 5 failed (pre-existing unrelated `test_fixtures_endpoint.py` environment gap per A110/A111) -- zero regressions.

**Not built:** Asian Handicap and Cards priority tiers -- both excluded, not currently recommendable markets (`schema.py:106`; `backtest.py:116` "cards aren't a tradeable market here"); adding them needs market-support work first, tracked as a prerequisite rather than folded into A115. Also not built: any code-enforced check that the LLM actually honored tier ordering on a conflict -- today this is prompt instruction only, unverified beyond spot review.

Full record: `documents/agent_user_stories.md` A115-A116.

## 39. Home/Away Goals Market — Live and Backtest Parity (W199, 2026-09-19)

New market: `home_goals`/`away_goals`, a fixed 1.5-line over/under on one team's own goal count -- distinct from `total_goals`, which is the combined score of both teams. Shipped end-to-end across 17 commits (`ec8a55a..cbee54d`): schema (`src/agent/schema.py`), all 4 prompt-posture variants, grading in both Python (`market_resolution.py`) and its TypeScript mirror (`MatchUI.tsx`), live odds fetch, and backtest/agent-train parity. Direct continuation of §28's own investigation, which first confirmed real odds exist for this market on both vendors but left it unbuilt pending a region-cost decision.

**Live serving.** `app/backend/odds_api_client.py` gained `team_totals` outcome parsing, keyed off the same per-event `GET /v4/sports/{sport}/events/{id}/odds` call `totals`/`btts` already use (§28's established pattern), but with one addition: `get_event_odds()` now accepts a `regions` override so this specific market can request the wider `uk,us,us2` bookmaker set §28's W199 investigation notes found necessary for reliable coverage (SP1 needed `us`/`us2` specifically -- BetMGM/FanDuel), while `totals`/`btts` stay on the cheaper default region set, keeping the `markets x regions` cost increase scoped to only the market that needs it. Before writing the parser, `scripts/verify_odds_api_team_totals.py` made a real live call against an actual fixture to confirm the assumed outcome shape -- a `description` field naming which team the line belongs to -- was exactly right, rather than guessing at the JSON shape from documentation alone.

Code review during this story caught a real forward-compatibility bug: `app/backend/scheduler_wiring.py`'s `PersistingOddsClient`/`FallbackOddsClient` wrapper classes sit between `eod_batch.py`/`t30_refresh.py` and the real `OddsAPIClient`, and neither wrapper was forwarding the new `regions`/`home_team`/`away_team` parameters through to the underlying client -- silently dropping them would have meant this market's region override never actually reached the live API call in production. This is the identical bug class that file already documents as W99 (a new client parameter added to the real client but not threaded through its own wrappers); fixed before it could repeat.

`app/backend/eod_batch.py`'s `add_secondary_odds()` fetches `team_totals` via a second, independent `get_event_odds()` call (its own region override, separate from the `totals`/`btts` call), with its own cache-reuse tracking -- a cache row that already checked `total_goals`/`btts` freshness still gets a one-time backfill check for `home_goals`/`away_goals`, so an EOD run from before this market existed doesn't silently skip it forever. `src/agent/graph.py` surfaces the fetched odds in the LLM's prompt, following the same pattern §28 established for `total_goals_odds` -- populating `match_info` alone is not sufficient without the matching prompt sentence.

**Backtest / agent-train.** No historical column exists for this market in `raw_matches` (football-data.co.uk doesn't publish it), so, as with BTTS/corners, the source is OddsPapi rather than the live-only Odds API. OddsPapi's `/v4/markets` metadata directly labels market ID 10226 as `marketType: "teamtotals-team1"` (handicap 1.5) and 10242 as `marketType: "teamtotals-team2"` (handicap 1.5) -- no line-parsing ambiguity, the vendor's own metadata names the line and side. Which side is home and which is away still needed independent verification: team1=home was confirmed by cross-referencing `participant1Name`/`participant2Name` from a live `/v4/fixtures` call against `data/oddspapi_snapshots/fixture_metadata.json`'s own `home`/`away` fields, across 9 different real fixtures, zero mismatches. This provenance is now recorded directly in `scripts/find_oddspapi_team_goals_market_ids.py` (the script that did the `/v4/markets` lookup) so it isn't only findable in past conversation history.

With the IDs pinned, `scripts/extract_oddspapi_odds_lookup.py` was extended to pull `home_goals`/`away_goals` lines from the already-downloaded 1,005-match historical corpus -- no new API pull required, since the corpus was already fetched for BTTS/corners/total_goals. Real coverage: 769/861 matches (~89%) for home, 720/861 (~84%) for away. `src/agent/backtest.py`'s `_build_match_info()` threads this into both `agent-backtest` and `agent-train` via the one shared function both already route through, so no separate wiring was needed for the two callers.

**Scope note on team-name matching.** The original plan speculated this market would need `TeamNameMapper` cross-source name normalization, the same class of gap BUG-015 documents elsewhere in this codebase for football-data.org-vs-Odds-API name mismatches. The actual implementation resolves home/away by direct string equality instead: both sides of a `team_totals` outcome comparison come from the same Odds-API event payload, not two different vendors, so there is no cross-source spelling gap to bridge here -- confirmed correct by the same live capture (`scripts/verify_odds_api_team_totals.py`) used to validate the outcome shape.

**Scope note: no `SnapshotStore` involvement, no ongoing re-pull.** Direct user question (2026-09-19): does the backtest side need any form of snapshotting? No, on both readings of the word. (1) `SnapshotStore` -- the tool-call record/replay mechanism `agent-snapshot`/`agent-backtest`/`agent-train` use to avoid re-calling the LLM/web-search/forecast tools -- is never touched here: `home_goals_odds`/`away_goals_odds` are injected in `_build_match_info()` via a plain static dict lookup (`data/oddspapi_btts_corners_odds.json`), not a tool call, so this data flows identically regardless of `SnapshotStore`'s mode, the same as `total_goals_odds`/`btts_odds`/`corners_odds` already do. (2) The OddsPapi historical *data* snapshot (the 1,005-file corpus itself) also needed no new pull -- it already existed from the earlier BTTS/corners work (A100), and Task 11 above only re-ran the existing extraction script against those same already-downloaded files to pull two more market IDs out of data already on disk.

Full story-level detail lives in `documents/app_user_stories.md` W199.

## 40. Generalized Already-Priced-In Evidence Guard (A117, 2026-09-19)

Direct user concern, surfaced while reviewing overlap between the ML model's feature set (rolling form, table position, market/odds features) and the agent's websearch evidence (`MATCH_CONTEXT_SEARCH_RESULT`/`FORM_SEARCH_RESULT`/`ODDS_VERIFICATION_SEARCH_RESULT`): A116 only taught the LLM to avoid double-counting a *stale injury* against `FORM_SEARCH_RESULT`. Every other overlapping evidence category had no equivalent guard, and the confidence rubric's "conflicting signals" trigger (line 68/69) didn't distinguish a genuine outside-the-model contradiction from the LLM's own restated phrasing of a feature the forecast already has landing as more negative/positive than the number itself — risking the LLM invalidating (or artificially reinforcing) a correct forecast based on redundant information dressed up as a new signal.

**Prompt-only fix, all 4 files (`agent_v1.txt` + 3 posture variants), identical edit, same pattern as A115/A116:** a new paragraph immediately after the A116 injury-recency paragraph generalizes the principle — if `MATCH_CONTEXT_SEARCH_RESULT`/`FORM_SEARCH_RESULT`/`ODDS_VERIFICATION_SEARCH_RESULT` merely restates something the forecast's own features already capture (recent results, league position, current market price), it's not new evidence and must neither raise nor lower the read, nor count as a "conflicting signal." Only evidence outside the model's inputs (a fact specific to the upcoming fixture, or something too recent to be in the historical record) should move it. The confidence-guideline line was tightened from "conflicting signals" to "evidence that genuinely contradicts the forecast with information outside its features" for the same reason.

No schema, field, query, or code change — reasoning-weight instruction only, same class of change as A115's priority tiers; no code-enforced check that the LLM honors it beyond prompt content, consistent with A115's own "not built" caveat.

Full record: `documents/agent_user_stories.md` A117.

## 41. Live `total_corners` Odds — OddsPapi Becomes a Second Live Vendor (W236, 2026-09-19)

Direct user report: `total_corners` never appeared as a recommendation on the frontend, despite being fully wired in schema (`src/agent/schema.py`), grading (`market_resolution.py`), the prompt (`config/prompts/agent_v1.txt`), and the frontend's own labels/pick-text maps. Traced to a live-odds gap, not a frontend filter: §28 above (A69, 2026-08-21) had called btts/corners a "permanent" gap since no source — live or historical — carried either. That's since narrowed: A100 (2026-08) built a one-off historical OddsPapi pull for backtesting (btts, corners, and later home/away goals per §39/W199), and this story adds the missing live half specifically for corners, since The Odds API (the vendor used for every other live market) has never carried a corners market at all, confirmed live.

**New live client, first of its kind for this vendor.** Every prior OddsPapi integration (`scripts/pull_oddspapi_btts_corners.py`, `extract_oddspapi_odds_lookup.py`) only ever wrote a static JSON snapshot for backtesting — nothing called OddsPapi live. `app/backend/oddspapi_client.py`'s `OddsPapiClient` is the first: `get_fixtures(tournament_id, status_id=1)` lists upcoming fixtures (confirmed free — no credit gating, per A100's own investigation notes on fixture-discovery cost); `get_corners_odds(fixture_id)` calls `/v4/odds?fixtureId=...&bookmakers=pinnacle` — the same live endpoint and JSON response shape (`bookmakers.pinnacle.markets[marketId].outcomes[outcomeId].players["0"]` price ticks) W199's investigation independently confirmed, and the exact parsing `extract_oddspapi_odds_lookup.py` already proved against real downloaded snapshots (market ID `10803`, fixed 9.5 line). Credit-gated by the same `CreditCounter` class `OddsAPIClient` uses, scaled to OddsPapi's much smaller 250-req/month free tier (`safety_margin=10` vs. The Odds API's 50/500).

**Wiring mirrors the existing home/away-goals precedent exactly.** `eod_batch.py`'s `add_secondary_odds()` gained a fourth independent cache-reuse block (own `already_checked_corners` tracking) alongside the existing total_goals/btts and home_goals/away_goals blocks — a cache row that already checked corners with an unchanged h2h price reuses it for free, which matters far more here than for The Odds API given OddsPapi's much tighter budget. Fixture matching (football-data.org match → OddsPapi's own `fixtureId`) reuses the identical `TeamNameMapper`/`config/team_mapping.json` convention BUG-015 established for The Odds API, via new `oddspapi_fixture_lookup()`/`matched_oddspapi_fixture_id()` helpers — OddsPapi spells clubs independently too, so this isn't optional. `run_eod_batch()`'s new `oddspapi_client` parameter defaults to `None`, so every caller that hasn't wired it (T-30 refresh, deliberately left unwired for now — see below) is unaffected, corners simply omitted, same contract `odds_client=None` already has.

**Construction:** `scheduler_wiring.build_oddspapi_client()` reads `ODDSPAPI_API_KEY`, wraps the client in a `PersistingOddsPapiClient` (own `FileCreditCounterStore`, mirroring `build_odds_client()`'s persistence pattern) so a restart doesn't lose the month's usage count, and returns `None` during sandbox mode — there's no OddsPapi equivalent of `HistoricalOddsClient`, since corners has no historical replay source reachable for a genuinely past match (A100's own unresolved finding, §28 above). Wired into both `main.py` call sites (the nightly scheduler job and the manual pregenerate endpoint).

**Deliberate scope cut, flagged to the user before merge:** T-30 refresh (`t30_refresh.py`) is not wired to fetch corners at all yet, even though it shares `add_secondary_odds()`. Since it reuses the exact same cache-reuse logic, wiring it in would mostly just re-read the EOD-cached value (a wasted `get_fixtures()` call, not a wasted credit) rather than genuinely refreshing — judged not worth the added parameter-threading yet.

Because OddsPapi's free tier is 250 requests/month per key and every match only needs the fetch once per generation cycle (cache-reuse above), corners recommendations will only appear for the subset of fixtures the active budget actually covers.

### 41.1 Multi-Key Fallback (`ODDSPAPI_API_KEY_2`/`_3`, follow-up same day)

Direct user follow-up, after being handed the real math: 5 leagues' combined fixture volume (~176 matches/month, from real per-league season match counts) leaves only ~27% headroom against a single key's 240 usable credits (250 minus the 10-credit safety margin) — tight enough that a busier stretch (odds-move re-fetches, manual `force=True` regenerations) could plausibly exhaust it some months. User opted to add a second key rather than run that close to the edge.

`scheduler_wiring.py` gained `FallbackOddsPapiClient`, mirroring `FallbackOddsClient`'s exact contract: tries each wrapped `PersistingOddsPapiClient` in order, moving to the next once one is locally exhausted (`get_corners_odds()`/`get_fixtures()` returns `None`) or actually rejected by the API (a raised `RequestException`). `build_oddspapi_client()` now builds one `PersistingOddsPapiClient` per configured `ODDSPAPI_API_KEY`/`_2`/`_3` (each with its own `FileCreditCounterStore` path), returning the single client directly when only one key is set (unchanged behavior) or a `FallbackOddsPapiClient` when more than one is. **One deliberate divergence from `build_odds_client()`'s own `ODDS_API_KEY_3` precedent:** `ODDSPAPI_API_KEY_3` does NOT default to the primary key when unset — that default was a specific BUG-056 dev-convenience workaround (so a local setup with only one real key could still exercise the 3-key code path), not a general convention worth reintroducing here; an unset `ODDSPAPI_API_KEY_2`/`_3` here is simply omitted from the chain.
...
### 41.3 §41's Live Client Never Actually Worked — Wrong `statusId` Default (W200, 2026-09-22)

W236 (§41 above) shipped a working-looking live OddsPapi client, verified at merge time — but the P0 backlog for this session independently listed `total_corners` live odds as still an open gap (`W200`), three days after W236's own "completed" status. Checked directly rather than trusting either status line: 43 real live fixtures (2026-09-19→09-22, all 5 leagues) via `GET /api/recommendations/{match_id}` carried **zero** `total_corners` candidates, while every other secondary market (`total_goals`/`btts`/`home_goals`/`away_goals`) appeared normally in the same batch.

Root cause: `OddsPapiClient.get_fixtures()`'s `status_id: int = 1` default (`app/backend/oddspapi_client.py`) — the one call site `eod_batch.py` uses to discover which OddsPapi `fixtureId` corresponds to tomorrow's not-yet-kicked-off match — returns **zero fixtures, for every league, unconditionally**. Confirmed via a direct `/v4/fixtures` sweep across `statusId ∈ {0,1,2,3}`: only `0` (`statusName: "Pre-Game"`) returns real fixtures (330 for E0 alone); `1` 404s `FIXTURE_NOT_FOUND` every single time. `1` is presumably OddsPapi's `Live/In-Play` status — structurally never true for a fixture that hasn't kicked off yet, which is the only case this code path is ever asked about. `scripts/pull_oddspapi_btts_corners.py`'s own historical pull correctly uses `statusId=2` and was never affected.

**This means §41's entire live corners pipeline has been silently non-functional since the day it shipped** — every downstream piece (the throttle, the credit-tracking, the multi-key fallback, the W238 fixture-matching fix) was built and tested correctly, but `oddspapi_fixture_lookup()` always received an empty fixture list to match against, so `get_corners_odds()` was structurally unreachable in production. `hasOdds` on the fixture record and the per-fixture odds payload were never actually inspected at merge time — only the fixture-matching and parsing logic were, against fixtures obtained a different way — which is how this shipped and stayed green through W238's own follow-up without being caught.

**Fix**: default corrected to `status_id: int = 0` in the one real call site plus its two pass-through wrappers (`PersistingOddsPapiClient`/`FallbackOddsPapiClient`, `app/backend/scheduler_wiring.py`). Two pre-existing tests had encoded the wrong default as an assertion (`test_oddspapi_client.py::test_get_fixtures_sends_correct_url_and_params_and_costs_no_credit`, `test_scheduler_wiring.py`) and were updated; confirmed both were RED against the corrected value before editing them, not just changed blind.

**Live verification.** Fixture discovery: 0 → 330/311/330/261/270 real upcoming fixtures (E0/SP1/I1/F1/D1). All 5 leagues happen to be mid-international-break right now (nearest resumption 2026-10-09/10, cross-checked against this project's own `/api/fixtures` independently — zero E0 rows 09-23→10-09), so no fixture yet has a real pinnacle corners price to fetch (checked directly: the nearest fixture has zero pinnacle markets posted at all, any market — consistent with bookmakers only pricing corners close to kickoff, not a further defect). The fix is verified correct at the actual, measurable defect (fixture discovery, the thing `status_id` controls); real corners prices reappearing in live recommendations should follow automatically as fixtures approach kickoff, with no further code change expected. Full suite: 1773 passed / 1 skipped, same 8 pre-existing/unrelated failures as this session's baseline.

Full record: `documents/app_user_stories.md` W200.

## 42. home_goals/away_goals API Gap, Stuck-Null Odds Cache, and a Direct User Match Reflection (2026-09-20)

Direct user request to check tomorrow's live recommendations surfaced two real bugs before any prompt work even started, then a written match-by-match reflection on a real batch of results surfaced five more findings -- three fixable via prompt, one via a new deterministic agent-schema guardrail, and two via app-layer/frontend fixes. Full story records: `documents/app_user_stories.md` W239-W242, `documents/agent_user_stories.md` A118-A122.

**W239 -- home_goals/away_goals silently dropped by the API layer.** `src/agent/schema.py` got the `home_goals`/`away_goals` market (plus `over_1.5`/`under_1.5` selections) on day one of W199 (2026-09-19), but `app/backend/recommendations.py`'s own `MarketCandidateOut`/`RecommendationPickOut` `Literal` types were never updated to match -- every home/away-goals candidate the agent produced failed pydantic validation in `validate_and_degrade()` and was counted as "malformed data from the agent," regardless of how good its edge was. Fixed by extending both `Literal`s and adding the market to `_SHAP_POSITIVE_SELECTION`. Confirmed live: 0 -> 20 home/away-goals candidates across a real 20-match batch after deploy + regen.

**W240 -- a stuck-null odds cache, found investigating why I1/D1/F1 still had zero home/away-goals candidates after W239.** Ruled out two plausible causes with real evidence before finding the actual one: (1) vendor coverage -- disproven, a direct query to The Odds API for the exact I1/D1/F1 fixtures showed FanDuel/BetMGM genuinely pricing `team_totals`; (2) credit exhaustion -- disproven, both `ODDS_API_KEY`/`_2` showed ~153/500 used via the vendor's own `x-requests-used` header, nowhere near the safety margin. The real cause: `add_secondary_odds()`'s cache-reuse gates (`already_checked_secondary`/`already_checked_team_totals`/`already_checked_corners`) checked whether an odds key was *present* in the cached dict, not whether its value was a real price -- a book that hadn't posted a line yet at cache time stored an explicit `None`, and since h2h staying unchanged never forces a re-check, that `None` replayed forever even once the book later posted a real price. Fixed by switching all three gates to a truthiness check on the cached value. Confirmed live: 6 -> 12 of 20 matches after the next scheduled EOD pass, now including previously-stuck D1/I1 fixtures.

**A118 -- FORM_SEARCH_RESULT competition contamination, flagged during the W240 investigation.** `research_node`'s form query (`"{home} {away} recent form last 5 matches"`, `pipeline.py`) doesn't scope itself to the match's own competition, so a "recent form" summary routinely blends league, cup, and European results with nothing in the prompt telling the LLM to notice. Prompt-only fix, all 4 posture files: check whether FORM_SEARCH_RESULT identifies which competition each result came from, and hedge with vaguer phrasing rather than assert an unverified league-specific count. Its live confirmation came the very next day in A120 below.

**The match reflection (2026-09-20).** Direct user wrote up a per-match critique of a real batch: Fiorentina v Napoli, Man City v Sunderland, Parma v Genoa, Bournemouth v Liverpool, Leverkusen v RB Leipzig, Atlético v Real Madrid, plus one general UX complaint. Each item was checked against the real final scores (Fiorentina 1-1, City 5-3, Parma 2-1, Bournemouth 0-1, Leverkusen 2-0) before deciding whether it was a genuine bug, and if so whether prompt, agent-node, or neither was the right layer:

- **A119 (prompt): total_goals narrative can contradict the model's own home_goals/away_goals numbers.** Fiorentina v Napoli's `total_goals/over_2.5` pick credited "won their last two, scored seven goals" to Fiorentina's own attack in `team_evidence`, but the model's own `home_goals` candidate for Fiorentina favored UNDER 1.5 (edge -0.106 over / +0.044 under) -- the real driver was Napoli's `away_goals` number. The actual score (1-1, both sides under 1.5) matched the model's per-side reads, not the narrative's attribution. Fix: check the credited side's own home_goals/away_goals probability before attributing a total_goals lean to it.
- **A120 (prompt): two availability-reading traps.** A Parma player described only as an unselected substitute ("on the bench") was cited as unavailable -- bench/rotation is a team-selection choice, not unavailability. Separately, a Napoli centre-back (Buongiorno) with zero appearances all season was still cited as a fresh injury concern -- A116's existing long-standing-absence guard wasn't phrased strongly enough for the "never played this season at all" extreme. Both stated explicitly in a new paragraph after A116's.
- **A121 (prompt) + a creative data-gap workaround: squad depth and FIFA ratings as a replacement-quality proxy.** Man City v Sunderland's `total_goals/under_2.5` pick weighted Foden's suspension and Doku's injury as flat negatives despite City's real squad depth -- real result 5-3 (8 goals), the worst miss of the batch. No live structured squad/appearance lookup exists in this codebase (FotMob `SQUAD_*` features are training-time-only, not queryable at generation time), and building one is a real new data-source integration, out of scope here. Direct user's own proposed fix needed no new code at all: spend the agent's existing one optional `web_search` call comparing the missing player's and their likely replacement's FIFA/EA Sports FC card overall rating as a rough quality-gap proxy when replacement quality materially affects a close-to-recommending market.
- **A122 (new agent-schema guardrail, not prompt-only): composite_score can be objectively self-contradictory, not just internally inconsistent.** `composite_score` is the LLM's own self-report with no fixed formula (A88/A91) -- the existing `_downgrade_recommendation_below_top_composite_score` (A91) only catches a contradiction *within* the composite_score column (a rejected candidate self-rating higher than the pick), never a contradiction between composite_score and the ml_probability/value_edge numbers it's supposed to summarize. Two real matches in the same batch proved the gap: Parma v Genoa picked `result_3way/away` (ml=0.450, edge=+0.050) over `total_goals/over_2.5` (ml=0.546, edge=+0.120) -- the rejected candidate beat the pick on *both* axes, yet self-reported a lower composite_score; Bournemouth v Liverpool showed the identical shape (`btts/no`, ml=0.554/edge=+0.184, rejected in favor of `result_3way/away`, ml=0.533/edge=+0.079). Neither is the prompt's legitimate "smaller edge, materially higher probability" tradeoff -- strict dominance on both axes has no such judgment call to make regardless of what composite_score is supposed to mean. New `_downgrade_pick_dominated_by_another_candidate()` (`src/agent/schema.py`), wired into the downgrade chain immediately after A91's check: downgrades to `no_bet` (never auto-promotes the dominator, same reasoning A91 already uses -- that candidate's own `reason`/`team_evidence` text was never written to justify it as the pick) when any other still-eligible candidate strictly dominates on both `ml_probability` and `value_edge`. A tie on either axis alone is not dominance, matching A91's own `>`-not-`>=` convention.
- **W241 (frontend): page-switching lag.** `api.ts` had zero client-side caching -- `AppShell`, `DashboardPage`, and `MatchExplorerPage` each independently call `getFixtures()`/`getCachedRecommendation()`/`getStatus()` on mount. New 15s-TTL in-memory cache keyed by request path on the four read-only GET functions; mutations untouched, `generateRecommendation` invalidates its own match/date entry so a regenerate is never shadowed by a stale read.
- **W242 (frontend): "2 Direct Bets" badge ambiguity.** Leverkusen v RB Leipzig had both `result_3way/home` and `away_goals/under_1.5` individually flagged `direct_bet` (composite_score 0.71 vs. 0.8) -- `ProbabilityRow` rendered both identically via `StatusBadge`, which read as two official recommendations on one match. `StatusBadge` gained `isThePick` (default `true`): `false` on a non-highlighted row relabels it "Also Qualifies" instead of reusing the pick's own wording.
- **Investigated and not pursued:** the Atlético v Real Madrid "despite yellow cards pointing the other way" quote could not be located in the current cached recommendation (likely superseded by a later T-30 refresh regen before the check ran) -- no fix without the original text. Verifying a specific backup goalkeeper's real quality, and verifying a named player is still actually on a club's current roster (not transferred), both need a live structured squad/roster lookup this codebase doesn't have -- out of scope as a prompt/node fix, tracked as a known gap rather than built speculatively.

**Test/verification summary.** W239: 611 backend tests. W240: 1741 (root `tests/` + `app/backend/tests/`). A119-A122 (prompt files + new guardrail): 1747 passed / 1 skipped, plus 6 new `tests/test_agent_dominance_guardrail.py` tests and one existing self-consistency test fixture corrected (its `tied` candidate happened to also strictly dominate the pick on the raw axes, independent of its composite_score tie -- adjusted to isolate the composite_score-tie behavior it was actually testing). W241/W242: 336 frontend tests. All runs share the same 5 pre-existing/unrelated `test_fixtures_endpoint.py` failures and 1 pre-existing/unrelated `BetTracker.race.test.tsx` failure, both confirmed failing on the pre-change baseline via `git stash`.

## 43. `btts`/`no` Suppressed Project-Wide — `_downgrade_direct_bet_btts_no()` (US#210, 2026-09-22)

`documents/user_stories.md` US#210/US#207/US#192 record the full ML-side investigation: `btts:no`'s real edge on qualifying bets is severely negative in all 5 leagues (E0 -24.0%, SP1 -31.5%, D1 -11.8%, F1 -29.1%, I1 -34.5%), and three independent training-time levers were each ruled out with real held-out evidence via the `US#205` leak-free gate (`src/utils/real_edge_gate.py::check_real_edge`) before this fix: a freshly-fit, held-out-gated calibrator (rejected by its own out-of-sample check); every `time_decay_half_life_days` from 0 to 730 days (real edge unchanged, -32% to -37% throughout); and, this session, every `sample_weight_alpha` from 0.0 to 1.0 (also unchanged, `scripts/sweep_btts_alpha.py`) — ruling out the `result_3way`/draw-style class-balance overcorrection (Section 27.3-adjacent's now-retired draw floor) this doesn't actually share a root cause with. Per-bet inspection of I1's 23 qualifying `no` bets found the actual mechanism: raw `predict_proba()` for `btts` barely leaves a **0.47-0.55** band regardless of the match, while the real market (`implied_no`) ranges 0.23-0.46 — i.e. confidently priced `yes` in every one of those 23 matches (all 23 resolved `yes`). The model's near-constant ~50% output mechanically clears the `value_edge` floor on `no` exactly when it has the least real information, not when it has found a genuine countervailing signal. This is a feature/information gap (the market prices on team news/injuries this project's engineered stats don't carry — the same gap `US#175`'s lineup-absence flag and `US#208`'s SoFIFA player-quality proxy are queued to close), not a tunable parameter — so unlike every guardrail in Sections 27.3-27.5 (a config-gated threshold), this fix is a flat, always-on downgrade with no knob to tune.

**Implementation.** New `_downgrade_direct_bet_btts_no(data)` (`src/agent/schema.py`), added to `extract_recommendation()`'s existing downgrade chain immediately after the `result_3way`/draw floor (Section 27.3-adjacent) and before A28's null-odds pass: any candidate with `recommendation_type == "direct_bet"`, `market == "btts"`, and `selection == "no"` is downgraded straight to `no_bet`, with a `limitations` note. Same shape as A28's `_downgrade_direct_bet_with_null_odds` (BUG-013) — unconditional, no `AgentConfig` field, no `compute_agent_config_hash()` change needed. `btts`/`yes` and every other market/selection are untouched.

**Verification.** New `tests/test_agent_btts_no_suppression.py` (5 tests): downgraded even at a large (0.16) value_edge since there's no threshold to clear; `btts`/`yes` unaffected; market-scoped (doesn't key off `selection == "no"` alone, mirroring the draw floor's own market-scoping defense); an already-`no_bet` candidate untouched; a self-reported `conditional` candidate left to the existing conditional-eligibility pipeline rather than double-handled. Three pre-existing tests that incidentally used a `btts`/`no` candidate as a generic second-candidate fixture for unrelated logic needed updates: `tests/test_agent_self_consistency_guardrail.py`'s `_BTTS` fixture switched to `selection="yes"` (its own composite-score/self-consistency behavior under test doesn't depend on which selection it is), and `tests/test_agent_conditional_market_eligibility.py` dropped `("btts", "no")` from its ineligible-market parametrize list (that scenario's initial `direct_bet` candidate can no longer reach A29's conditional-eligibility check at all now that this pass intercepts it first — the other 8 parametrized cases still cover that check). Full suite green at the same 8 pre-existing, unrelated `app/backend/tests/` failures as `US#205`'s own baseline (date-range-dependent fixture mocks).

## 44. Cross-Vendor Roster Mismatch and Silent Stale-Team-History Bug (W198, 2026-09-22)

`documents/app_user_stories.md` W198's original scope was narrow: `football-data.org` was listing SP1 fixtures for "Real Racing Club de Santander," a club `football-data.co.uk`'s own current-season CSV (`raw_matches`' real feature source) never tracked, producing repeated `Unmapped team` warnings for one fixture. Investigated end-to-end rather than accepting the original framing at face value, since the story itself flagged the root cause as unknown.

**Not a division mixup.** Queried `football-data.org`'s `/v4/competitions/PD/teams` directly — a real, self-consistent 20-team top-flight roster comes back, `currentSeason` correctly matches. The actual cause is a **genuine 3-vs-3 roster disagreement between the two vendors**: `football-data.org`'s PD roster has Málaga CF, RC Deportivo La Coruña, and Real Racing Club de Santander where `football-data.co.uk`'s `SP1_2526.csv` has Girona, Mallorca, and Real Oviedo instead — the co.uk roster matches real-world 2025-26 La Liga, `football-data.org`'s doesn't. Real impact is much larger than one fixture: **108/380 SP1 matches this season (28.4%)** involve one of these 3 clubs (`/v4/competitions/PD/matches`), 88 still upcoming.

**A second, more severe bug, found investigating why "Deportivo" didn't even reproduce the original warning.** Racing Santander has zero `raw_matches` rows under any spelling — correctly trips the existing `US#108` zero-history `unknown_team` guard. But Málaga and Deportivo (mapped to `raw_matches`' own historical spelling, "Malaga"/"La Coruna") **do** have real rows — every one dated 2016-08 to 2018-05, their actual last La Liga season, ~8 years stale relative to "today." `FeatureFactory.build_for_match()`'s zero-history check only ever verified *existence* of a row, never recency, so both clubs were silently treated as fully known (`unknown_team: false`, `feature_completeness: 0.917` confirmed on a real live check) — computing rolling-window features from an 8-year-old roster/era with zero relationship to the current team. **Confirmed live, not theoretical**: `Getafe v Deportivo` (2026-09-13) served `result_3way/draw` as `direct_bet` (`ml_probability=0.4518`) on this exact basis; `Deportivo v Real Betis` (2026-09-20) served `result_3way/away` as `direct_bet` before a separate, unrelated guardrail switched the final pick to a different market.

**Fix.** `src/features/feature_factory.py::build_for_match()` gained a staleness check alongside the existing zero-history one: `_MAX_STALE_HISTORY_DAYS = 400` (comfortably above any normal in-season/off-season gap, not backtested/tuned — a deliberately conservative round number). A team whose most recent `raw_matches` row predates the match being forecast by more than that is now folded into the same `unknown_team`/`_unknown_team` signal the zero-history case already uses — **and its own stale rows are dropped from the rolling-feature input entirely**, not just relabeled, so its rolling features come back `NaN` and get cold-start imputed like a genuinely unseen team, while the other side's real, fresh history (if any) is untouched. Implemented as a targeted row-filter (`raw_df` minus the stale team's own home/away rows) right before the existing "no history at all" branch, so both cases converge on the same downstream cold-start path.

**Verification.** New `tests/test_stale_team_history_flag.py` (3 tests): a stale team (real rows, all 8 years old) is flagged `unknown_team`; a fresh team isn't; and — the test that actually proves the fix, not just the flag — a stale team's feature row is asserted numerically identical (`pd.testing.assert_frame_equal` over every shared numeric column) to a genuinely-zero-history team's row, given the same opponent, proving the stale rows contribute nothing rather than just being mislabeled. Confirmed RED against the pre-fix code first (2 of 3 tests failed, including the numeric-equality one, with a real ~2-goal difference in `OFF_HOME_FTHG_EMA5` between the stale and zero-history cases). Live-verified: re-running `build_for_match` for the exact real Deportivo v Real Betis matchup that had already produced a live `direct_bet` now returns `unknown_team: True`. Full suite: 1776 passed / 1 skipped, same 8 pre-existing/unrelated failures as this session's baseline.

**Not addressed here, flagged as a candidate follow-up**: `unknown_team`/`cold_start_risk` is prompt-only guidance outside the `conservative` posture variant — no hard code-level gate stops a `direct_bet` from being issued on a flagged match, which is exactly how the two live examples above got through. Worth a dedicated look given this session found a real instance of it, but a guardrail redesign is out of scope for this story's own remit (the data-quality root cause).

Full record: `documents/app_user_stories.md` W198.
