...
**Deliberate scope cuts, both flagged to the user before merge:**
1. Single `ODDSPAPI_API_KEY` only — no `_2`/`_3` multi-key fallback chain like `OddsAPIClient` has. That complexity was added there reactively, after real quota exhaustion was observed in production; the same pattern applies here if this budget is ever actually observed running dry, not proactively.
2. T-30 refresh (`t30_refresh.py`) is not wired to fetch corners at all yet, even though it shares `add_secondary_odds()`. Since it reuses the exact same cache-reuse logic, wiring it in would mostly just re-read the EOD-cached value (a wasted `get_fixtures()` call, not a wasted credit) rather than genuinely refreshing — judged not worth the added parameter-threading yet.

Because OddsPapi's free tier is 250 requests/month total and every match only needs the fetch once per generation cycle (cache-reuse above), corners recommendations will only appear for the subset of fixtures the monthly budget actually covers — an accepted, explicit tradeoff (user's own call), not a bug.
