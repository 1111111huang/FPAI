...
- `src/agent/snapshot_store.py` — `league_base_dir()` (the `<base_dir>/<LEAGUE>/` helper the `agent-snapshot` CLI already uses) and `SnapshotStore._path()` (confirms `_complete.json`-per-match-directory is the existing "is this match fully recorded" convention — reuse it, don't invent a new one).
