...
def test_run_agent_includes_corners_odds_in_prompt_when_present():
    """A100: corners_odds (from _build_match_info's OddsPapi lookup) must
    reach the LLM's prompt the same way total_goals_odds/btts_odds already
    do -- populating match_info alone is a no-op if the model never sees it."""
    from unittest.mock import MagicMock, patch
    from langchain_core.messages import AIMessage, HumanMessage
    from src.agent.graph import run_agent
    from src.agent import tools as agent_tools
    ...
    with patch("src.agent.graph._build_llm") as mock_build_llm, \
        ...
        run_agent(
            match_info={
                "home_team": "Man City", "away_team": "Arsenal", "date": "2026-06-21", "league": "E0",
                "odds": {"home": 2.0, "draw": 3.4, "away": 3.6},
                "corners_odds": {"over_9.5": 1.9, "under_9.5": 1.95},
            },
            config=cfg,
            tools=[],
        )
    ...
    messages = mock_llm.bind_tools.return_value.invoke.call_args[0][0]
    prompt_message = next(m for m in messages if isinstance(m, HumanMessage) and "Analyse the upcoming match" in m.content)
    assert "over_9.5=1.9" in prompt_message.content
    assert "under_9.5=1.95" in prompt_message.content
