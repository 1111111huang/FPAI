...
def test_e0_enabled_feature_groups_resolves_to_full_schema_feature_set() -> None:
    ...
    assert len(kept) == 185  # US#176 (+6 MKT_LINE_MOVE/MKT_BOOK_DISAGREEMENT) + US#175 (+2 LINEUP_) + US#174 (+6 DC_) = 167 + 14 + US#178 (+6 DC_CORNER_) = 181 + 6 -- US#179 (-6 MKT_LINE_MOVE/DISAGREEMENT, train/serve skew) = 187 - 6 + US#208 (+4 SQUAD_*_MKT_VALUE_MEAN) = 181 + 4
...
def test_e0_via_model_manager_resolves_to_full_schema_feature_set() -> None:
    ...
    assert len(features) == 185  # US#176 (+6) + US#175 (+2) + US#174 (+6) + US#178 (+6) = 167 + 20 -- US#179 (-6 MKT_LINE_MOVE/DISAGREEMENT, train/serve skew) = 187 - 6 + US#208 (+4 SQUAD_*_MKT_VALUE_MEAN) = 181 + 4
