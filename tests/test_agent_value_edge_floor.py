...
_VALID_CANDIDATE = {
    ...
    "value_edge": -0.08,
    "composite_score": 0.6,
    "reason": "Below the value-edge floor, testing the downgrade path.",
}
...
def test_direct_bet_with_negative_value_edge_downgraded_to_no_bet():
    ...
    candidate = {**_VALID_CANDIDATE, "current_odds": 1.30, "value_edge": -0.138}
...
def test_direct_bet_at_exact_floor_is_accepted():
    candidate = {**_VALID_CANDIDATE, "value_edge": 0.05}
    data = {**_VALID, "candidates": [candidate]}
...
def test_direct_bet_above_floor_is_untouched():
    candidate = {**_VALID_CANDIDATE, "value_edge": 0.12}
    data = {**_VALID, "candidates": [candidate]}
...
def test_no_bet_market_below_the_floor_is_not_touched():
    candidate = {**_VALID_CANDIDATE, "recommendation_type": "no_bet", "value_edge": -0.3}
...
def test_custom_min_value_edge_is_respected_not_hardcoded():
    candidate = {**_VALID_CANDIDATE, "value_edge": 0.06}
    data = {**_VALID, "candidates": [candidate]}
...
def test_runs_before_a29_so_a_negative_edge_market_never_becomes_conditional():
    ...
    candidate = {**_VALID_CANDIDATE, "current_odds": 1.05, "value_edge": -0.138}
    data = {**_VALID, "candidates": [candidate]}
