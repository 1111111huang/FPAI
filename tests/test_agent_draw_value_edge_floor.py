...
_DRAW_CANDIDATE = {
    ...
    "value_edge": 0.134,
    "composite_score": 0.6,
    "reason": "Draw looks priced generously versus the model's own probability.",
}

_VALID_PICK = {"market": "result_3way", "selection": "draw"}

_VALID = {
    "match": {"home": "Liverpool", "away": "Nottingham", "date": "2026-08-27", "league": "E0"},
    "overall": "direct_bet",
    "candidates": [_DRAW_CANDIDATE],
...
def test_draw_above_the_dedicated_floor_is_untouched():
    candidate = {**_DRAW_CANDIDATE, "value_edge": 0.22, "ml_probability": 0.42}
...
def test_draw_at_exact_dedicated_floor_is_accepted():
    candidate = {**_DRAW_CANDIDATE, "value_edge": 0.15}
    data = {**_VALID, "candidates": [candidate]}
...
def test_home_and_away_result_3way_selections_are_not_affected():
    ...
    for selection in ("home", "away"):
        candidate = {**_DRAW_CANDIDATE, "selection": selection, "value_edge": 0.08}
        data = {**_VALID, "candidates": [candidate], "recommendation_pick": {"market": "result_3way", "selection": selection}}

        rec = extract_recommendation(_wrap_json(data), min_value_edge_result_3way_draw=0.15)

        assert rec["candidates"][0]["recommendation_type"] == "direct_bet", selection
...
def test_other_markets_draw_named_selection_not_affected():
    ...
    selection alone)."""
    candidate = {**_DRAW_CANDIDATE, "market": "btts", "selection": "yes", "value_edge": 0.08}
...
def test_no_bet_draw_market_is_not_touched_by_this_pass():
    ...
    candidate = {**_DRAW_CANDIDATE, "recommendation_type": "no_bet", "value_edge": -0.3}
...
def test_conditional_draw_market_is_downgraded_by_a54_not_by_this_pass():
    ...
    candidate = {**_DRAW_CANDIDATE, "recommendation_type": "conditional", "value_edge": -0.3}
...
def test_runs_after_the_general_floor_so_a_negative_edge_draw_cites_the_general_reason():
    ...
    candidate = {**_DRAW_CANDIDATE, "value_edge": -0.1}
    data = {**_VALID, "candidates": [candidate]}
