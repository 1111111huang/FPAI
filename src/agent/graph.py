    ...
    def output_node(state: AgentState) -> dict:
        ...
        if not text.strip():
            ...
            synthesis_prompt = (
                ...
                "Include all required fields: match, overall, candidates, recommendation_pick, explanation, confidence, limitations, prediction_basis. "
...
def run_agent(
    ...
    # 2026-08-21: total_goals had exactly one real odds source (raw_matches's
    # own over25_odds/under25_odds, threaded in by backtest.py's
    # _build_match_info for replay only) -- btts/corners had no equivalent
    # real column, live or historical. W164 (2026-08-25) added a second,
    # live source for both total_goals and btts: eod_batch.py's
    # _generate_one now calls OddsAPIClient.get_event_odds() (the per-event
    # endpoint -- the bulk one 422s for these two markets, confirmed live)
    # and sets these same two match_info keys for real live fixtures, not
    # just backtest replay. Corners still has no real source either way --
    # see documents/agent_techspec.md's "Secondary-market odds coverage"
    # section.
    total_goals_odds = match_info.get("total_goals_odds")
    if total_goals_odds:
        over = total_goals_odds.get("over_2.5")
        under = total_goals_odds.get("under_2.5")
        prompt += f" Bookmaker odds for total goals (over/under 2.5): over_2.5={over}, under_2.5={under}."
    btts_odds = match_info.get("btts_odds")
    if btts_odds:
